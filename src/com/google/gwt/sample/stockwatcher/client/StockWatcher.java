...
public class StockWatcher implements EntryPoint {
	private static final int REFERESH_INTERVAL = 5000; //ms
	...
	public void onModuleLoad() {
		// Check login status using login servicea.
	...
	private void loadStockWatcher() {
		...
		refreshTimer.scheduleRepeating(REFERESH_INTERVAL);
	...
	private void addStock() {
		...
		if (stocks.contains(symbol)) {
			Window.alert("'" + symbol + "' is not already added.");
			return;
		}
	...
	private void updateTable(StockPrice[] prices) {
		...
		DateTimeFormat dateFormat = DateTimeFormat.getFormat(DateTimeFormat.
	...
	private void updateTable(StockPrice price) {
		...
		if (price.getChange() < 0.0) {
			changeStyleName = "negativeChange";
		} else if (price.getChange() > 0.0) {
